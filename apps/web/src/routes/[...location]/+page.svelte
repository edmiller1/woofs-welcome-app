...
<phantom-ui loading={location.isLoading}>
  ...
  {/if} -->
    ...
    {#if location.isSuccess}
      ...
      <main>
        <!-- Hero Section -->
        <section
          class="relative h-180 w-full overflow-hidden flex items-end px-8 md:px-16 pb-20"
        >
        ...
        <section class="bg-surface-subtle">
          ...
          {#if location.data.adventures.length > 0}
            <div class="max-w-7xl mx-auto px-8 pb-24">
              ...
              {:else}
                <p class="text-muted-foreground text-sm">
                  No adventures listed yet for {location.data.name}.
                </p>
              {/if}
            </div>
          {/if}
        ...
        <section class="max-w-full mx-auto md:px-24 py-24">
          ...
          >
            <div class="absolute bottom-10 right-2 z-50">
              <Button
                href="/{pathname}/explore"
